...
use Quicktane\Core\Base\Dto\Attributes\Map;
...
use Quicktane\Core\Base\Dto\Contracts\BaseDTO;
use Quicktane\Core\Base\Dto\Exceptions\CastTargetException;
use Quicktane\Core\Base\Dto\Exceptions\MissingCastTypeException;
use ReflectionClass;
...
abstract class SimpleDTO implements BaseDTO, CastsAttributes
{
    ...
    private function buildAttributesData(): void
    {
        ...
        $mapDataProperties = $this->getPropertiesForAttribute($publicProperties, Exclude::class);
        foreach ($mapDataProperties as $property => $attribute) {
            $this->dtoExclude[] = $property;
        }
    }
    ...
    private function buildDataForExport(): array
    {
        ...
        return $this->removeExcluded($this->mapDTOData($mapping, $this->validatedData));
    }

    private function removeExcluded(array $data): array
    {
        return Arr::except($data, $this->dtoExclude);
    }
